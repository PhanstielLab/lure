...
awk '{if ($4 <= 80) print $0}' /tmp/lure/rprobes.bed | grep -Evi 'AAAAAAAAAA|TTTTTTTTTT|GGGGGGGGGG|CCCCCCCCCC' > /tmp/lure/rtemp0.bed
awk -v OFS="\t" '{if ($10 >= 0.5 && $10 <= 0.6) print $0, 0}' /tmp/lure/rtemp0.bed | awk -F "\t" '!seen[$5, $6]++' > /tmp/lure/rtemp1.bed
...
awk -v OFS="\t" '{if ($10 >= 0.4 && $10 <= 0.7) print $0, 1}' /tmp/lure/rtemp0.bed | awk -F "\t" '!seen[$5, $6]++' > /tmp/lure/rtemp2.bed