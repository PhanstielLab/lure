...
grep -Evi 'AAAAAAAAAAAAAAAAAAAA|TTTTTTTTTTTTTTTTTTTT|GGGGGGGGGGGGGGGGGGGG|CCCCCCCCCCCCCCCCCCCC' "$output_folder/rprobes.bed" | awk -v OFS="\t" '{if ($10 >= 0.4 && $10 <= 0.7) print $0, 2}' | awk -F "\t" '!seen[$5, $6]++' > "$output_folder/rtemp3.bed"
